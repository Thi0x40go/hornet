...
async fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    if !app.active_conn_id.is_empty() {
        ...
        if let Ok(ping) = app.db.ping(&active).await {
            app.conn_health
                .insert(active.clone(), (ping.online, ping.latency_ms));
        }
        ...
        if event::poll(poll_dur)? {
            match event::read()? {
                Event::Key(key) => {
                    ...
                    if app.show_new_conn_modal {
                        match key.code {
                            ...
                            KeyCode::Left if app.new_conn_field == 1 => {
                                ...
                                app.new_conn_url =
                                    app::DRIVER_TYPES[app.new_conn_type_idx].2.to_string();
                            }
                            KeyCode::Right if app.new_conn_field == 1 => {
                                app.new_conn_type_idx =
                                    (app.new_conn_type_idx + 1) % app::DRIVER_TYPES.len();
                                app.new_conn_url =
                                    app::DRIVER_TYPES[app.new_conn_type_idx].2.to_string();
                            }
                            KeyCode::Char(c) => match app.new_conn_field {
                                0 => app.new_conn_name.push(c),
                                1 => {
                                    if let Some(idx) = app::DRIVER_TYPES
                                        .iter()
                                        .position(|(d, _, _)| d.starts_with(c))
                                    {
                            ...
                            KeyCode::Backspace => match app.new_conn_field {
                                0 => {
                                    app.new_conn_name.pop();
                                }
                                2 => {
                                    app.new_conn_url.pop();
                                }
                                _ => {}
                            },
                            KeyCode::Enter => {
                    ...
                    {
                        let note_name = app.current_note.clone().unwrap_or_else(|| {
                            format!("query_{}", chrono::Local::now().format("%Y%m%d_%H%M%S"))
                        });
                    ...
                    // Pane-specific keys
                    match app.focus {
                        FocusArea::Drawer => match key.code {
                            KeyCode::Up | KeyCode::Char('k') => {
                        ...
                        FocusArea::Editor => {
                            match app.vim.mode {
                                VimMode::Normal => {
                                    ...
                                    if let Some(op) = app.vim.pending_op.take() {
                                        match (op, key.code) {
                                            ...
                                            ('d', KeyCode::Char('$')) => {
                                                ...
                                                app.vim.is_line_register = false;
                                                app.editor_lines[app.cursor_row] =
                                                    cur_line[..app.cursor_col].to_string();
                                            }
                                            ('c', KeyCode::Char('c')) => {
                                                app.vim.save_undo(
                                                    &app.editor_lines,
                                    ...
                                    match key.code {
                                        ...
                                        KeyCode::Char('o') => {
                                            ...
                                            app.editor_lines
                                                .insert(app.cursor_row + 1, String::new());
                                            app.cursor_row += 1;
                                            app.cursor_col = 0;
                                            app.vim.enter_insert(
                                                &app.editor_lines,
                                        ...
                                        KeyCode::Char('l') | KeyCode::Right => {
                                            let max_c = app.editor_lines[app.cursor_row]
                                                .len()
                                                .saturating_sub(1);
                                        ...
                                        KeyCode::Char('b') => {
                                            let cur_line = &app.editor_lines[app.cursor_row];
                                            app.cursor_col =
                                                prev_word_start(cur_line, app.cursor_col);
                                        }
                                        KeyCode::Char('e') => {
                                            let cur_line = &app.editor_lines[app.cursor_row];
                                        ...
                                        KeyCode::Char('^') => {
                                            let cur_line = &app.editor_lines[app.cursor_row];
                                            app.cursor_col = cur_line
                                                .find(|c: char| !c.is_whitespace())
                                                .unwrap_or(0);
                                        }
                                        KeyCode::Char('G') => {
                                            app.cursor_row =
                                                app.editor_lines.len().saturating_sub(1);
                                            app.cursor_col = 0;
                                        }
                                        KeyCode::Char('g') => {
                                        ...
                                        KeyCode::Char('x') => {
                                            ...
                                            if app.cursor_col < cur_line.len() {
                                                ...
                                                app.cursor_col = app
                                                    .cursor_col
                                                    .min(cur_line.len().saturating_sub(1));
                                            }
                                        }
                                        // Half-page scrolls
                                        KeyCode::Char('d')
                                            if key.modifiers.contains(KeyModifiers::CONTROL) =>
                                        {
                                            app.cursor_row = (app.cursor_row + 8)
                                                .min(app.editor_lines.len().saturating_sub(1));
                                        }
                                        KeyCode::Char('u')
                                            if key.modifiers.contains(KeyModifiers::CONTROL) =>
                                        {
                                        ...
                                        KeyCode::Char('p') => {
                                            if !app.vim.register.is_empty() {
                                                ...
                                                if app.vim.is_line_register {
                                                    app.editor_lines.insert(
                                                        app.cursor_row + 1,
                                                        app.vim.register.clone(),
                                                    );
                                                    app.cursor_row += 1;
                                                    app.cursor_col = 0;
                                                } else {
                                                    let cur_line =
                                                        &mut app.editor_lines[app.cursor_row];
                                                    let pos =
                                                        (app.cursor_col + 1).min(cur_line.len());
                                                    cur_line.insert_str(pos, &app.vim.register);
                                                    app.cursor_col = pos
                                                        + app.vim.register.len().saturating_sub(1);
                                                }
                                            }
                                        }
                                        KeyCode::Char('P') => {
                                ...
                                VimMode::Visual | VimMode::VisualLine => match key.code {
                                    KeyCode::Esc => {
                                    ...
                                    KeyCode::Char('y') => {
                                        if let Some((anchor_r, _)) = app.vim.visual_anchor {
                                            let (r_start, r_end) = if anchor_r <= app.cursor_row {
                                                (anchor_r, app.cursor_row)
                                            } else {
                                                (app.cursor_row, anchor_r)
                                            };
                                    ...
                                    KeyCode::Char('d') | KeyCode::Char('x') => {
                                        ...
                                        if let Some((anchor_r, _)) = app.vim.visual_anchor {
                                            ...
                                            let removed: Vec<String> =
                                                app.editor_lines.drain(r_start..=r_end).collect();
                                            ...
                                            app.cursor_row =
                                                r_start.min(app.editor_lines.len() - 1);
                                            app.cursor_col = 0;
                                        }
                                        app.vim.enter_normal(
                                            app.cursor_row,
                                    ...
                                    match key.code {
                                        ...
                                        KeyCode::Up => {
                                            if app.cursor_row > 0 {
                                                ...
                                                app.cursor_col = app
                                                    .cursor_col
                                                    .min(app.editor_lines[app.cursor_row].len());
                                            }
                                            app.show_completions = false;
                                        }
                                        KeyCode::Down => {
                                        ...
                                        KeyCode::Left => {
                                            if app.cursor_col > 0 {
                                                ...
                                                app.cursor_row -= 1;
                                                app.cursor_col =
                                                    app.editor_lines[app.cursor_row].len();
                                            }
                                            app.show_completions = false;
                                        }
                                        KeyCode::Right => {
                                        ...
                                        KeyCode::Backspace => {
                                            if app.cursor_col > 0 {
                                                let mut line =
                                                    app.editor_lines[app.cursor_row].clone();
                                        ...
                                        KeyCode::Delete => {
                                            ...
                                            if app.cursor_col < line_len {
                                                ...
                                                let next_line =
                                                    app.editor_lines.remove(app.cursor_row + 1);
                                                app.editor_lines[app.cursor_row]
                                                    .push_str(&next_line);
                                            }
                                        }
                        ...
                        FocusArea::Results => match key.code {
                            KeyCode::Up | KeyCode::Char('k') => {
                            ...
                            KeyCode::Char('L') | KeyCode::Char(']') => {
                                if let Some(res) = &app.query_result {
                                    app.selected_col = (app.selected_col + 4)
                                        .min(res.headers.len().saturating_sub(1));
                                }
                            }
